...
class SteeredModelWrapper:
    ...
    This class mimics a basic LangChain chat model interface (via `invoke`).
    """
    ...
    def register_hook(self, layer_idx: int, vector: torch.Tensor, coeff: float = 1.0) -> None:
        ...
        def hook_fn(module: torch.nn.Module, input_tensors: Tuple[torch.Tensor, ...], output: Tuple[torch.Tensor, ...] | torch.Tensor) -> Tuple[torch.Tensor, ...] | torch.Tensor:
            ...
            # Ensure the vector is on the correct device and dtype to match the model's forward pass
...
# We instantiate `response_model` as a module-level singleton here. 
# Loading a 2B parameter model takes significant time and memory. In a full production
# system, this might be handled via dependency injection or FastAPI's lifespan events,
# but for this specific orchestrator script, a singleton prevents reloading the model on every request.
try:
...
class LocalRouter:
    """
    A fast, deterministic router that analyzes user input
    to determine which intent-based steering tools to invoke.
    """
    def __init__(self):
        import re
        self.trigger_patterns = {
            r"\b(cat|kitten|feline|test)\b": "mention_cat_tool",
            r"\b(joy|happy|cheerful|excited|glad|smile)\b": "joyful_tool",
            r"\b(surprise|random|weird|crazy)\b": "surprise_me_tool",
            r"\b(uncertain|confidence|doubt|unsure|maybe|perhaps|guess)\b": "lower_confidence_tool",
            r"\b(reset|clear|default|normal)\b": "reset_steering_tool"
        }
        self.compiled_patterns = {re.compile(p, re.IGNORECASE): tool for p, tool in self.trigger_patterns.items()}

    def invoke(self, messages: List[BaseMessage]) -> AIMessage:
        """Analyzes the recent message history and generates tool calls if regex patterns match."""
        last_msg = messages[-1]
        
        # Avoid infinite loops: if the last message was a tool output, stop steering.
        ...
        for pattern, tool_name in self.compiled_patterns.items():
...
router_model = LocalRouter()

def router_call(state: AgentState) -> Dict[str, BaseMessage]:
    """Graph node: Decides which steering tools to invoke."""
    response = router_model.invoke(messages=list(state["messages"]))
